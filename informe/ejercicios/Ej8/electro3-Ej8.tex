...
nuevamente por el sensor. Luego, el sensor calcula el tiempo que tardó
en retornar la señal ultrasónica emitida y devuelve este resultado
provocando un pulso de echo de la misma duración que el tiempo calculado
anteriormente.

Entonces, lo que mide el sensor enrealidad es tiempo que puede ser
...
Por último, una especifiación importante es que el sensor es capaz
...
\begin{figure}[H]
\begin{centering}
\includegraphics[scale=0.4]{pinOutGenerico.PNG}
\par\end{centering}
...
Para el diseño se procuró indentificar las distintas etapas necesarias
...
proyecto a realizar se puede representar con la siguiete imagen:

\begin{figure}[H]
\begin{centering}
\includegraphics[scale=0.4]{diagramaDeBloquesGenerico.PNG}
\par\end{centering}
...
HC SR04, se comenzará analizando los módulos mas ``cercanos'' al
...
con la señal de ``echo'' del mismo y esta la cual debe analizar.
...
Por ùtlimo, para este módulo se debe tener en cuenta que el contador
...
\begin{figure}[H]
\begin{centering}
\includegraphics[scale=0.6]{esquemaProcesamientoDeEcho.PNG}
\par\end{centering}
\begin{centering}
...
El estado de este pin será definido únicamente por el estado el echo
...
pierda debido a que el módulo que la proceso comienze a relizar otras
...
nueva medición. Por ende, con el módulo de almacenamiento de información
se logra obtener información de una medición anterior mientras se
está realizando una nueva medición. Luego, la salida de este módulo
...
Esta indicación sobre la activación del disparo conformará la entrada
del módulo y tan solo basta con que sea un bit que represente un ``1''
lógico en caso de acitvar un disparo o un ``0'' en caso de no pretender
una medición.
...
\begin{figure}[H]
\begin{centering}
\includegraphics[scale=0.6]{generadorDePulsoMenor50us.PNG}
\par\end{centering}
...
\begin{figure}[H]
\begin{centering}
\includegraphics[scale=0.6]{habilitacionDeDisparo.PNG}\caption{Diseño Circuital - Habilitación de Disparo}
\par\end{centering}
\end{figure}
...
\begin{figure}[H]
\begin{centering}
\includegraphics[scale=0.4]{pinOutLM55(a).PNG}\includegraphics[scale=0.4]{pinOutLM55(b).PNG}
\par\end{centering}
...
Como se puede deducir a partir de las imagenes anteriores, al excitar
...
es adecuada ya que es 5 veces mayor la ancho mínimo del pulso requerido
...
y representa, en cambio, un ``0'' lógico cuando se mantiene descargandose
entre $\frac{2}{3}$ y $\frac{1}{3}$ de $V_{cc}$. A continuación
se presenta el cálculo del tiempo que de encendido de la señal (1
...
Además, se a partir de lo calculado se obtiene que $f_{clock}=\frac{1}{t_{ON}+t_{OFF}}\simeq\frac{1}{2ln(2)R_{2}C_{2}}=\frac{1}{ln(4)R_{2}C_{2}}$.
...
Luego, esto se puede lograr con $R_{1}=1K\Omega$, $R_{2}=72K\Omega$
y $C_{1}=1nF$
...
este que se puede identificar como mejorables.
...
de 4 bits con su lectura y escirtura en modo paralelo, dado que los
...
conformando bloques sólidos que tenian como característica su capacidad
de ser testeado o probado de una manera simple.

Por otro lado, el diseño también fue un desafío ya que antes de tomar
una desición o definir una idea surgieron muchas variantes para una
